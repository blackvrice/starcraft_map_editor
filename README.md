...
> 현재 상태: M1 데스크톱 기반, M2 CHK 무손실 코어와 M3 맵 아카이브 입출력을
> 완료했습니다. 원본을 변경하지 않는 `.scm/.scx` 열기, raw `scenario.chk`
> 왕복과 진단, 검증형 Save As, 외부 변경 감지와 기존 출력의 복구 백업을
> 지원합니다. M4 EUD 수직 기능은 euddraft 설치·버전 검사, 격리된 외부
> 프로세스 실행 경계, 안전한 빌드 설정 모델과 변경 상태를 표시하는 최소
> epScript 편집기까지 구현했으며 파일 저장과 EUD 빌드 파이프라인은 아직
> 구현 중입니다.
