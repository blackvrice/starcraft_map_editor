...
class ObjectEditingController {
  ...
  ChkTechSettings get techSettings {
    ...
    if (session == null || !_isEditableSession(session)) {
      throw StateError('Open an editable map before changing techs.');
    }
    return const ChkTechSettingsEditor().read(session.rawDocument);
  }

  void applyTechSettings({
    required RawChkDocument expectedDocument,
    required Map<TechSettingKey, int> changes,
  }) {
    final session = openMapController.state.session;
    if (session == null ||
    ...
    final replacements = const ChkTechSettingsEditor().edit(
      session.rawDocument,
      changes,
    );
