...
void main() {
  testWidgets('lists catalog entries and keeps unplaceable ones visible', (
    ...
    await _pumpPane(tester, fixture.controller);
  ...
  testWidgets('Place confirms the selection and returns to the map', (
    tester,
  ) async {
    final fixture = await _fixture();
    addTearDown(fixture.dispose);
    final events = await _pumpPane(tester, fixture.controller);
    await tester.tap(find.byKey(const Key('placement-catalog-tab-doodad')));
    await tester.pumpAndSettle();
  ...
  testWidgets('Back to map leaves without arming a placement', (tester) async {
    final fixture = await _fixture();
    addTearDown(fixture.dispose);
    final events = await _pumpPane(tester, fixture.controller);
    await tester.tap(find.byKey(const Key('placement-catalog-tab-doodad')));
...
/// Tracks the callbacks the shell uses to switch back to the map tab.
final class _PaneEvents {
  int confirmed = 0;
  int closed = 0;
}

Future<_PaneEvents> _pumpPane(
  WidgetTester tester,
  PlacementCatalogController controller,
) async {
  final events = _PaneEvents();
  await tester.pumpWidget(
    MaterialApp(
      home: Scaffold(
        body: PlacementCatalogPane(
          controller: controller,
          onPlacementConfirmed: () => events.confirmed++,
