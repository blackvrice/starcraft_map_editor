...
후속 [공식 배포본 대조](UPSTREAM_COMPARISON.md)로 Rust 확장과 libffi를 포함한
23개 파일의 배포 출처를 해시로 연결했다. 위 버전 리소스가 없는 5개라는 관찰은
유지하지만, 그중 Rust 확장과 libffi는 더 이상 배포 출처 자체가 미확인인 상태는 아니다.
freezeMpq의 차이와 전이 의존성 고지는 여전히 미확인이다.
