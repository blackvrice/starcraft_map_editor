...
기존 `.eud.json` 덮어쓰기는 아직 거부한다. 외부 변경 검사·백업을 포함한 기존
프로젝트 저장과 선언적 설정 편집·공유 영향 분석·생성기 연결이 다음 개발 범위다.
...
`LocalEudProjectStore`는 새 파일로 Save As·다시 읽기를 지원한다. 기존 파일 교체는
아직 허용하지 않는다. 저장 실패는 현재 문서와 저장 기준선을 유지한다.
