...
첫 번째 수동 사례인 [공중 유닛 설정 비교 맵](SETTINGS_GAME_SMOKE.md)을 준비했다.
이 첫 사례는 사용자 테스트 완료 보고를 받았다. 상세 수치와 게임 build는 별도 기록이 없으며, 나머지 게임 검증은 미완료다.

후속 [무기·업그레이드·클로킹 비교](SETTINGS_COMBAT_SMOKE.md)의 생성·바이트·MPQ 검증을 추가했다. 실제 게임 결과는 확인 대기다.
