...
| M6.3.1. 유닛·무기 EUD 확장 | 진행 | 후보 필드·프로젝트 탭·파일 저장·맵 연결 검증 구현, 필드 편집·생성 빌드 대기 |
...
2026-09-14 사용자 우선순위에 따라 M6.3.1 EUD 확장 개발로 이동했다. M6.3의 무기·업그레이드·클로킹 게임 확인과 나머지 사례 검증은 대기 상태로 유지한다. EUD 프로젝트 저장·Undo 기반을 구현했다. 프로젝트 탭·현재 맵 fingerprint 연결을 구현했다. 2026-09-15 외부 변경 검사·복구 백업을 포함한 기존 프로젝트 Save를 완료했다. 다음은 선언적 필드 편집·공유 영향 분석이다. 실제 지원 활성화 전 도구·게임 검증을 별도로 완료해야 한다.
