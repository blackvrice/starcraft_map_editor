...
| M5. 맵 캔버스와 지형 | 진행 중 | 지형 typed view·탐색·자산 설정·MTXM 기본 편집 |
...
- [ ] 실제 타일 렌더링용 CascLib 읽기 경계와 캐시 ADR
...
- 실제 타일 렌더링 전, 현재 검사 전용 helper와 분리된 자산 읽기 계약을
  ADR로 확정한다. 고정된 40개 매니페스트 밖의 임의 CASC 경로는 허용하지
  않고 자산별·전체 크기 상한, 프로토콜 버전, 게임 빌드와 CascLib revision을
  검증한다. 전송 방식은 대용량 바이트를 JSON에 포함하지 않으며 게임 자산을
  영구 추출하지 않는 프로세스 스트림 또는 수명 제한 앱 소유 캐시로 제한한다.
- 픽셀 합성은 `MTXM`의 group/member로 `CV5` mega-tile을 선택하고,
...
- 합성 결과는 StarCraft 설치 fingerprint·게임 빌드·타일셋·raw 값을 키로
  캐시하고 설치 경로나 검사 revision이 바뀌면 폐기한다. 캔버스는 준비된
