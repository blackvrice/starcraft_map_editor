...
| M6. 객체와 로케이션 | 진행 중 | 객체 typed view·레이어·선택·검색·속성 Inspector |
| M7. 일반 트리거 | 대기 | 트리거 편집 |
...
- 팔레트 배치는 템플릿 레코드의 owner·flags·예약 필드를 그대로 복사하고 x/y
  ...
  유지하며 `Escape`나 레이어 전환으로 취소한다. 로케이션 생성은 별도 M6
  항목에서 문자열과 ID 정책을 함께 구현한다.
...
- class/relation/state flags, unused, 스프라이트 flags 같은 원시·예약 필드는
  ...
  표시하며 속성 Apply는 단일 선택으로 제한한다. 로케이션은 다음 M6 항목에서
  크기·이름·ID 정책을 함께 구현하기 전까지 Inspector에서 읽기 전용이다.
