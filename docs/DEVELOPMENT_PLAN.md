...
| M6. 객체와 로케이션 | 진행 중 | 객체 typed view·레이어 표시/잠금·선택 우선순위 |
| M7. 일반 트리거 | 대기 | 트리거 편집 |
...
- Open Map과 검증형 Save As는 객체 view를 다시 디코딩해
  `OpenedMapSession.objectViews`에 보관한다. 현재 항목은 읽기 전용 구조 기반이며
  좌표·플레이어·문자열 참조의 의미 검증과 쓰기 API는 뒤의 M6 항목에서 추가한다.
...
- 애플리케이션 계층은 typed view를 렌더 중립적인 point/region 장면으로 만들고,
  Presentation은 이를 단순 도형과 선택 강조로 표시한다. 현재 클릭은 첫 hit를
  검사 선택하는 범위이며 다중 선택·이동·삭제는 다음 M6 항목에서 구현한다.
