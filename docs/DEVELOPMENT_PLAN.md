...
2026-09-14 사용자 우선순위에 따라 M6.3.1 EUD 확장 개발로 이동했다. M6.3의 무기·업그레이드·클로킹 게임 확인과 나머지 사례 검증은 대기 상태로 유지한다. EUD 프로젝트 저장·Undo 기반을 구현했다. 프로젝트 탭·현재 맵 fingerprint 연결을 구현했다. 다음은 외부 변경 검사와 백업을 포함한 기존 프로젝트 저장 및 선언적 필드 편집·공유 영향 분석이다. 실제 지원 활성화 전 도구·게임 검증을 별도로 완료해야 한다.
...
- [ ] 외부 변경 검사·백업을 포함한 기존 프로젝트 저장 연결 (향후 스키마 변경 시 migration 구현)
