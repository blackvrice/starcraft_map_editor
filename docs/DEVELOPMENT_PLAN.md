...
| M6.3. 맵·플레이어·게임 설정 | 진행 | 맵 정보·플레이어·세력·유닛 기본 설정 구현, 무기 연결·생산 허용·게임 설정 대기 |
...
다음 구현 작업은 M6.3의 검증된 유닛–무기 연결과 공유 무기 변경 영향 목록이다. 그다음 플레이어별 유닛 생산 허용을 진행한다.
...
- [ ] 검증된 로컬 유닛–무기 연결을 제공하고 공유 무기 변경의 영향받는 유닛 목록 표시
