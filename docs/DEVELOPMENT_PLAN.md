...
| M3. 맵 아카이브 입출력 | 진행 중 | Open Map·검증형 Save As·fingerprint 완료, 백업 정책 대기 |
...
- `SaveMapController`는 Windows Save As 경로를 선택하고 최종 경로와 같은
  ...
  별도 백업 정책 항목에서 구현한다. 저장 시작 시 열린 세션의 fingerprint와
  원본을 비교하고, 검증된 임시 출력 승격 직전에 원본을 다시 확인한다. 변경이나
  삭제가 감지되면 임시 출력은 승격하지 않으며 성공으로 표시하지 않는다.
