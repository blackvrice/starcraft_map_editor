...
| M6.2. 시각적 배치 선택 탭 | 진행 | 카탈로그 선택·배치와 네 종류 실제 설치 자동 검증 완료, 기존 Doodad 복합 삭제·성능 기준선 남음 |
| M6.3. 맵·플레이어·게임 설정 | 진행 | 맵·플레이어·세력·유닛·생산 허용·업그레이드·테크 설정 구현, 검색·ID 범위 복사 구현, 통합 설정 탭 구현, 게임 검증 대기 |
| M6.3.1. 유닛·무기 EUD 확장 | 진행 | 후보 5개 필드 편집·저장·공유 영향·CHK 비교 구현, 실제 생성 빌드·게임 검증 대기 |
...
- [ ] 대형 카탈로그 가상 스크롤·검색·썸네일 cache 성능 계측
      (합성 Windows profile 첫 기준선·실제 Unit/Sprite 보유량 계측·64×64 상한 완료. profile 반복/실제 자산 UI·전체 cache 상한·GPU 메모리는 미검증)
