...
| M6.3.1. 유닛·무기 EUD 확장 | 진행 | 후보 필드·프로젝트 파일 저장·Undo 기반 구현, UI·빌드 연결 대기 |
...
2026-09-14 사용자 우선순위에 따라 M6.3.1 EUD 확장 개발로 이동했다. M6.3의 무기·업그레이드·클로킹 게임 확인과 나머지 사례 검증은 대기 상태로 유지한다. EUD 프로젝트 저장·Undo 기반을 구현했다. 다음 개발은 프로젝트 UI·현재 맵 연결이며, 실제 지원 활성화 전 도구·게임 검증을 별도로 완료해야 한다.
...
- [x] 선언적 v1 스키마·맵 경로/해시 참조·프로젝트 파일 Save As/다시 읽기·버전 거부·dirty/Undo 기반 구현 (ADR-0009, UI 미연결)
- [ ] 프로젝트 UI·현재 맵 fingerprint 대조·외부 변경 검사를 포함한 기존 프로젝트 저장 연결 (향후 스키마 변경 시 migration 구현)
