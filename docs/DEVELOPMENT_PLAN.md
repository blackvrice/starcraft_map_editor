...
| M6.2. 시각적 배치 선택 탭 | 진행 | 카탈로그 썸네일 상한·사용자 성능 확인, 검증형 단일 Doodad 복합 삭제 구현. 삭제의 실제 게임/외부 에디터 검증 남음 |
| M6.3. 맵·플레이어·게임 설정 | 진행 | 맵·플레이어·세력·유닛·생산 허용·업그레이드·테크 설정 구현, 검색·ID 범위 복사 구현, 통합 설정 탭 구현, 게임 검증 대기 |
| M6.3.1. 유닛·무기 EUD 확장 | 진행 | 선언적 설정의 결정적 생성·명시적 안전 테스트 빌드 연결, 게임 검증 대기 |
| M6.3.2. 설정별 EUD 확장 | 진행 | 8개 분류·61개 후보 필드 편집·저장·테스트 빌드, 공유 그래픽 영향·게임 검증 대기 |
| M6.4. 기본 제작·편집 도구 | 대기 | 새 맵·등각 지형·안개·클립보드·객체 속성 보완 |
| M6.5. 문자열·사운드 리소스 | 대기 | 사용처와 안전한 편집·아카이브 리소스 관리 |
| M7. 일반 트리거·미션 브리핑 | 진행 | 일반 조건 22종·액션 57종, 소유자/활성/순서·스위치·UPRP·저장 전 참조 검사 구현. 브리핑·리소스 사용처·게임 검증 남음 |
...
2026-09-14 사용자 우선순위에 따라 M6.3.1 EUD 확장 개발로 이동했다. M6.3의 무기·업그레이드·클로킹 게임 확인과 나머지 사례 검증은 대기 상태로 유지한다. EUD 프로젝트 저장·Undo 기반을 구현했다. 프로젝트 탭·현재 맵 fingerprint 연결을 구현했다. 2026-09-15 외부 변경 검사·복구 백업을 포함한 기존 프로젝트 Save를 완료했다. 후보 5개 필드 편집·공유 영향 분석·CHK 비교를 완료했다. 2026-09-16 확장 계획을 정리했으며 다음 EUD 개발은 X1 도구 동봉 기반부터 시작한다. 실제 지원 활성화 전 도구·게임 검증을 별도로 완료해야 한다.
...
기본 순서는 이 단계와 M6.3~M6.5 이후 M7이다. 2026-09-27 사용자 지시에 따라 M7 첫 구현을 먼저 진행하며 선행 단계의 미확인 항목은 그대로 유지한다.
...
- [x] Doodad의 `DD2 `·`MTXM`·필요한 `THG2` 원자적 배치와 검증형 단일 삭제/Undo 구현
      (2026-09-25: 유일한 TILE·일치한 recipe·명시적 overlay 귀속 확인에 한정. 겹침 가능성·disabled·다중 선택·복원 정보 부재는 전체 거부. [ADR-0010](decisions/0010-reviewed-doodad-deletion.md))
- [ ] Doodad 복합 삭제의 실제 게임/외부 에디터 왕복 검증과 미지원 사례 확대
...
- [x] 대형 카탈로그 가상 스크롤·검색·썸네일 cache 계측·상한 및 사용자 실제 성능 확인
      (2026-09-23 사용자 직접 성능 확인 완료. 2026-09-25 Tile 보유 RGBA 16MiB 제한·최근 사용 우선 해제·화면 재진입 재요청 구현. Unit/Sprite는 고정 개수×16KiB 상한. metadata·일시 atlas·외부의 이전 상태 참조·GPU는 payload 예산 제외. 자동 반복/GPU 원시 수치는 미제공.)
      2026-09-18: 상세 선택 변경 시 이전 썸네일 잔류 수정. 탭/대화상자 공통 디코더, 빠른 전환·종료 자원 해제 회귀 추가.
      컨트롤러 종료 시 보유 썸네일·선택·스냅샷 참조 해제 및 늦은 콜백/응답 차단 검증 완료.
      2026-09-20: 무효화된 객체 카탈로그의 후속 atlas 요청·썸네일 복사/축소 차단과 설치 변경·종료 회귀 추가.
      Tile 카탈로그에도 후속 atlas 요청·썸네일 추출 차단 적용. 설치·맵·종료 회귀와 실제 타일 배치 확인.
      2026-09-22: 같은 Profile 빌드의 실행별 출력과 최소/중앙/최대 집계 도구 추가. 실패·환경 불일치 거부 테스트 완료. Windows 앱 제어 정책 차단으로 반복 실측은 대기.
      2026-09-23: 항목 검색 문자열·불변 상태별 필터 결과 재사용으로 화면 재빌드 시 중복 검색/할당 제거. 2,000개 목록·검색 변경·페이지 추가 회귀 완료. Profile 실측·전체 cache/GPU 상한은 남음.
      Tile 썸네일 추출의 중간 픽셀 복사 2회 제거. 최종 4KiB 독립·불변 버퍼 및 입력 변경 격리 회귀 완료.
      카탈로그 페이지 간 중복/역순·전체 개수 변경·조기 종료의 누적 차단과 재로딩 복구 구현. 네 종류 12개 회귀 완료. 정상 전체 cache 상한은 별도 남음.
...
- [x] 플레이어 슬롯 종류·종족·색상과 시작 위치 참조 진단 구현
- [x] 세력 배정·이름·동맹·공동 승리·시작 위치 관련 옵션 편집 구현
- [x] 유닛 종류별 이름·최대 체력/실드·방어력·생산 비용/시간·무기 기본/추가 피해량과 기본값 복원 구현
- [x] 검증된 로컬 유닛–무기 연결을 제공하고 공유 무기 변경의 영향받는 유닛 목록 표시
- [x] 플레이어별 유닛 생산 허용과 기본값 상속 편집 구현
- [x] 업그레이드 비용/시간 및 증가량, 플레이어별 시작/최대 레벨과 기본값 편집 구현
- [x] 테크 비용/연구 시간/에너지, 플레이어별 허용·연구 완료와 기본값 편집 구현
- [x] 유닛·무기·생산 허용·업그레이드·테크 검색 및 ID 범위 초안 복사·취소·검증·Undo/Redo 구현
- [x] Map Settings 통합 탭, Players/Forces 공통 선택·플레이어 범위 및 Inspector 안내 구현
- [x] 설정별 golden bytes·미편집 값 보존 증거 정리와 실제 MPQ Save As/재열기 검증
- [x] 첫 공중 유닛 수치 비교 맵 생성·바이트/MPQ 검증과 수동 확인 안내 준비
- [x] 첫 공중 수치 비교 맵 실제 게임 확인 — 사용자 테스트 완료 보고
- [x] 무기·시작 업그레이드·클로킹 비교 맵 생성과 자동 검증·확인 안내 준비
- [ ] 무기·업그레이드·클로킹 실제 게임 확인과 생산·연구·플레이어/세력 나머지 사례 검증

검증 메모:

- 2026-09-09 [유닛 생산 허용](UNIT_AVAILABILITY.md)을 구현했다. 단일 PUNI의
  유닛 228개에 대해 맵 기본값·플레이어 1~8의 허용/상속을 편집하고 9~12는
  읽기 전용으로 보존한다. 상속 전환은 저장된 개별 값을 지우지 않는다.
  초안·Apply/Cancel·Undo/Redo, 잘못된 구조/값 거부, golden bytes와
  Save As/재열기 왕복을 검증했다. analyze와 Windows debug 빌드·시작은 통과했다.
  전체 테스트는 재실행에서 494개 통과·9개 선택 스모크 건너뜀이다.
  첫 실행은 기존 타일 테스트 로딩 중 Dart VM 내부 UTF8 오류로 종료됐다.
  저장소 전체 format 게이트는 기존 infrastructure 테스트 4개 때문에 실패하며
  이번 변경 파일의 포맷은 통과했다. 검증 SDK는 Flutter 3.47.2/Dart 3.13.2로,
  기준 3.44.8/3.12 및 실제 SC:R 생산 동작은 미검증이다.
...
### 플레이어 설정 구현 확인 (2026-09-09)

- File → Player Settings에서 플레이어 1~8의 슬롯·종족·기본 색상을 여러 슬롯에
  걸쳐 입력하고 한 명령으로 Apply한다. Cancel, 공유 Undo/Redo, Save As와
  오래된 문서 스냅샷 거부를 지원한다. [상세 지원 범위](PLAYER_SETTINGS.md).
- OWNR/SIDE/COLR의 변경한 바이트만 수정한다. 누락·중복·크기 오류는 해당 그룹을
  차단하고 비표준 기존 값과 예약 플레이어 9~12를 보존한다. CRGB가 있는 맵은
  색상 편집을 차단한다. 캔버스 색상 미리보기는 아직 소유자별 기본 색상을 쓴다.
- 시작 위치의 누락·중복·비활성 슬롯·잘못된 소유자를 비차단 경고로 표시한다.
  열기·편집·Undo/Redo·저장 재검증 시 갱신하며 자동 배치나 삭제는 하지 않는다.
- Flutter 3.47.2 / Dart 3.13.2에서 analyze, 전체 테스트 467개(환경 의존 9개
  skip), Windows Debug 빌드와 시작 스모크를 통과했다. 필수 포맷 검사는 기존
  infrastructure 테스트 4개 차이로 실패하며 이번 변경 파일은 포맷을 맞췄다.
  기준 SDK 3.44.8과 실제 SC:R 실행은 미검증이다. 저장 왕복은 합성 CHK와
  가짜 아카이브 gateway로 검증했으므로 게임 상호 운용 검증을 대신하지 않는다.

### 세력 설정 구현 확인 (2026-09-09)

- File → Force Settings에서 플레이어 1~8의 세력 배정과 세력 1~4의 이름,
  동맹·공동 승리·시야 공유·시작 위치 무작위화를 한 명령에 적용한다.
  Cancel, 공유 Undo/Redo, 오래된 스냅샷 거부와 Save As를 지원한다.
- 단일 20바이트 FORC에서 선택 필드만 수정하고 예약 비트·비표준 기존 배정을
  보존한다. 이름은 새 문자열 참조로 분리하여 공유 원문과 tail을 보존한다.
  이름 오류는 배정·옵션을 막지 않는다. [상세 범위](FORCE_SETTINGS.md).
- Flutter 3.47.2 / Dart 3.13.2에서 analyze, 전체 테스트 473개(환경 의존 9개
  skip), Windows Debug 빌드와 시작 스모크를 통과했다. 필수 포맷 게이트는
  기존 infrastructure 테스트 4개의 차이로 실패한다. 이번 변경은 포맷을 맞췄다.
- 기준 SDK 3.44.8과 실제 SC:R 로비·게임 실행은 이번에 검증하지 않았다.
  합성 CHK 및 가짜 gateway의 저장 왕복이므로 실제 게임의 동맹·시야·공동 승리와
  레거시 인코딩 검증은 남는다. 짧은 FORC의 확장·정규화는 지원하지 않는다.

### 유닛 기본 설정 구현 확인 (2026-09-09)

- File → Unit Settings에서 유닛 종류 0~227의 이름·최대 체력/실드·방어력·
  생산 비용/시간과 무기 ID별 기본/추가 피해량을 편집한다. 선택 간 입력 유지,
  일괄 Apply, Cancel, 공유 Undo/Redo, 기본값 복원과 Save As를 지원한다.
- 체력은 1/256 HP 정밀도를 보존하며 표현할 수 없는 소수를 거부한다. 기본값
  복원은 저장된 사용자 수치를 지우지 않는다. 버전별 UNIS/UNIx를 선택하고
  반대쪽 섹션·공유 이름 원문·원시 tail·미편집 값을 보존한다.
- 유닛–무기 연결이 현재 helper 계약에 없어 영향받는 유닛 목록은 제공하지
  못한다. 화면에 제한을 명시하고 별도 후속 체크박스를 추가했다. 게임 기본
  수치 표시와 실제 적용 검증도 남는다. [상세 지원 범위](UNIT_SETTINGS.md).
- Flutter 3.47.2 / Dart 3.13.2에서 analyze, 전체 테스트 483개(환경 의존 9개
  skip), Windows Debug 빌드와 시작 스모크를 통과했다. 기존 infrastructure
  테스트 4개 포맷 차이로 전체 포맷 게이트는 실패하며 이번 파일은 포맷을 맞췄다.
  기준 SDK 3.44.8과 실제 SC:R 실행은 미검증이다. 저장 왕복은 합성 CHK와 가짜
  아카이브 gateway로 검증했으므로 게임 상호 운용 검증을 대신하지 않는다.

### 로컬 유닛–무기 참조 목록 확인 (2026-09-09)

- Unit Settings에서 선택 무기의 직접 지상·공중 참조와 서브유닛 경로의 유닛 ID를
  구분해 표시한다. 중복·순환 참조를 처리하고, 불완전한 목록은 빈 목록으로
  표시하지 않는다. 설치·맵 변경과 종료 뒤 응답을 취소·폐기한다.
- 기존 catalog port에 unitMetadataOnly 요청과 응답 확인을 추가했다. helper는
  units.dat 하나만 읽고 검증된 ID를 반환하며 일반 배치 동작은 유지한다.
  [계약·범위·검증](UNIT_WEAPON_REFERENCES.md). 앞선 기본 설정 메모의 연결 목록
  미지원 제한은 이번에 해소했다. 주문·별도 생성 유닛·EUD 런타임 관계는 제외한다.
- Flutter 3.47.2 / Dart 3.13.2에서 analyze, 전체 테스트 489개(환경 의존 9개
  skip), Windows Debug 빌드·앱 시작, native CTest 5/5를 통과했다.
- 실제 SC:R s1 build 13515에서 한 파일 19876바이트, 228/228개 유닛 참조를
  확인했다. 실제 전투·EUD 동작 및 기준 SDK 3.44.8은 이번에 검증하지 않았다.
  포맷 게이트에는 기존 infrastructure 테스트 4개의 차이가 남아 있다.

### 업그레이드 설정 구현 확인 (2026-09-09)

- [업그레이드 설정](UPGRADE_SETTINGS.md)의 비용·시간·증가량 및 맵/플레이어별
  시작·최대 레벨·상속을 구현했다. UPGS/UPGx와 UPGR/PUPx를 버전별로 선택하고
  비용/레벨 그룹의 누락·중복·크기를 독립 검증한다. UPGx 예약 바이트 위치는
  오프셋 61이며 기존 조사 초안의 “끝 바이트” 설명을 바로잡았다.
- 기본값 전환 시 사용자 값을 보존한다. 초안·Apply/Cancel·공유 Undo/Redo,
  시작/최대 관계, 원시 값·반대 섹션 보존, golden bytes와 Save As/재열기를 검증했다.
- flutter analyze, 변경 파일 format, Windows debug 빌드·시작을 통과했다.
  전체 테스트는 501개 통과·9개 선택 스모크 건너뜀이다.
  전체 format 게이트는 기존 infrastructure 테스트 4개의 포맷 때문에 실패한다.
- 검증 SDK는 Flutter 3.47.2/Dart 3.13.2이며 기준 3.44.8/3.12는 미검증이다.
  실제 SC:R 적용과 업그레이드별 게임 레벨 상한은 후속 게임 검증에 남아 있다.
  다음 구현은 테크 비용·연구 시간·에너지와 플레이어별 허용·연구 완료·기본값이다.

### 테크 설정 구현 확인 (2026-09-09)

- [테크 설정](TECH_SETTINGS.md)의 비용·연구 시간·에너지, 맵/플레이어별
  허용·연구 완료·기본값 상속을 구현했다. TECS/TECx와 PTEC/PTEx를 버전별로
  선택하며 비용/상태 그룹의 누락·중복·크기를 독립 검증한다.
- 허용과 연구 완료를 독립 플래그로 다루고 기본값 전환 시 사용자 값을 보존한다.
  초안·Apply/Cancel·공유 Undo/Redo, 원시 값·반대 섹션 보존,
  원본/확장 golden bytes와 Save As/재열기 왕복을 검증했다.
- flutter analyze, 변경 파일 format, Windows debug 빌드·시작을 통과했다.
  전체 테스트는 508개 통과·9개 선택 스모크 건너뜀이다.
  전체 format 게이트는 기존 infrastructure 테스트 4개의 포맷 때문에 실패한다.
- 검증 SDK는 Flutter 3.47.2/Dart 3.13.2이며 기준 3.44.8/3.12는 미검증이다.
  실제 SC:R 연구·사용 동작과 개별 테크의 유효 수치 상한은 후속 검증에 남긴다.
  다음 구현은 설정 탭의 검색·선택 범위 적용과 공통 편집 UX·Inspector 구분이다.

### 설정 검색·ID 범위 적용 확인 (2026-09-10)

- [공통 설정 검색·ID 범위 복사](SETTINGS_EDITING_UX.md)를 유닛·무기·생산 허용·
  업그레이드·테크에 연결했다. 검색은 현재 선택·쓰기 범위를 자동 변경하지 않는다.
  유닛 사용자 이름/초안 검색을 지원하며 #ID는 정확한 ID를 찾는다.
- 현재 ID에서 수정한 필드만 명시한 대상 ID의 초안에 복사한다. 비용과 현재
  플레이어/맵 범위를 안내하고 다른 플레이어의 초안은 복사하지 않는다.
  실제 Apply는 기존 도메인 검증과 단일 Undo/Redo 명령을 사용한다.
- 범위 오류·중복 제거·검색의 선택 보존, 다섯 선택 영역의 섹션 바이트 보존,
  취소·Undo/Redo, 플레이어 범위 격리, 대상 레벨 오류의 원자적 거부를 검증했다.
  전체 테스트 518개 통과·9개 선택 스모크 건너뜀, analyze와 Windows debug
  빌드·시작을 통과했다. 기존 infrastructure 테스트 4개의 전체 format 차이는 남아 있다.
- 검증 SDK는 Flutter 3.47.2/Dart 3.13.2이며 기준 3.44.8/3.12와 실제 SC:R 적용은
  미검증이다. 기존 공통 UX 항목을 이번 ID 범위 구현과 후속 통합 작업으로 나눴다.
  다음은 Map Settings 통합 탭, Players/Forces 공통 선택·플레이어 범위 및 Inspector 안내다.

### 통합 설정 탭·플레이어 범위 확인 (2026-09-10)

- [Map Settings](MAP_SETTINGS_UI.md)에 7개 설정 탭을 통합했다. 탭 전환은 초안을
  유지하고 현재 탭만 Apply한다. 다른 탭의 적용/Undo로 오래된 초안은 재읽기 전
  적용을 차단하며, 미적용 초안이 남은 창을 닫을 때 폐기 여부를 확인한다.
- Players/Forces와 생산 허용·업그레이드·테크의 플레이어 선택에 검색과 1부터
  시작하는 범위 복사를 연결했다. 현재 선택에서 수정한 필드만 복사하고 예약
  플레이어와 맵 기본값을 복사 대상으로 허용하지 않는다. Inspector에 배치 객체와
  맵 전체 설정의 차이를 안내한다.
- 7개 신규 위젯 테스트로 범위별 바이트 보존·Undo/Redo, 탭 초안·충돌·닫기와
  900×700 레이아웃을 확인했다. analyze, Windows debug 빌드와 시작 스모크 통과.
  전체 테스트 525개 통과·9개 선택 스모크 건너뜀이다.
- 전체 format 게이트는 기존 infrastructure 테스트 4개의 포맷 차이로 실패하며
  이번 변경 파일의 포맷은 통과했다. Flutter 3.47.2/Dart 3.13.2로 검증했고
  기준 3.44.8/3.12 및 실제 SC:R 적용은 미검증이다.
- 다음 작업은 설정별 golden bytes·미편집 값 보존·Save As/재열기 증거를 정리하고
  실제 SC:R 적용을 검증하는 항목이다. M6.3의 게임 검증은 완료로 표시하지 않는다.

### 실제 아카이브 설정 왕복 확인 (2026-09-10)

- [설정 검증 기록](SETTINGS_VERIFICATION.md)에 설정별 golden bytes와 보존 테스트,
  실제 MPQ 저장 실행 방법 및 게임 검증 대조 항목을 정리했다.
- 기존 통합 편집 시나리오를 실제 MPQ helper·로컬 fingerprint·검증형 Save As에도
  실행한다. 한글/공백 경로, 원본 MPQ 전체 바이트, 재열린 CHK 전체 바이트 및
  설정별 예상 payload·미편집 데이터 보존을 확인했다. 집중 통합 테스트 3개 통과.
- 정적 분석과 변경 파일 포맷 통과. 전체 테스트 527개 통과·8개 선택 스모크 건너뜀.
  전체 포맷은 기존 infrastructure 테스트 4개의 차이가 남는다.
  Flutter 3.47.2/Dart 3.13.2 사용, 기준 SDK 3.44.8/3.12는 미검증이다.
- 실제 게임 화면 관찰 도구가 없어 SC:R 동작 확인은 미실행이다. 합성 저장 fixture는
  플레이 가능한 게임 검증 맵이 아니다. 다음 작업은 자체 제작 게임 시나리오 준비와
  실제 SC:R 설정 적용 검증이며, M6.3 전체 완료로 표시하지 않는다.

### 설정 작업 탭과 유닛명 표시 (2026-09-13)

- 사용자 요청으로 맵·Catalog 옆에 Map Settings 작업 탭을 추가했다. File 메뉴의
  통합 설정도 같은 탭을 선택한다. 설정에서는 좌우 객체 패널을 숨겨 공간을 확보한다.
- 작업 탭 전환 시 초안·선택·검색을 유지하며 Apply/Undo/Redo 후에도 설정 화면을
  유지한다. Close는 초안 폐기 확인 후 설정 상태를 초기화하고 맵으로 돌아간다.
- Units 목록은 사용자 이름 초안·맵 사용자 이름을 우선하고 없으면 기본 영문명과
  ID를 표시한다. 228개 ID를 BWAPI 공식 목록과 대조했으며 이름 검색을 검증했다.
  게임 언어별 번역을 불러오거나 기본 이름을 CHK에 쓰는 변경은 아니다.
- 후속 사용자 요청으로 생산 허용·무기·업그레이드·테크 선택과 무기 영향 유닛에도
  기본 이름을 표시한다. 이름 없는 슬롯은 ID를 유지하며 이름/#ID 검색을 검증한다.
- 전체 테스트 527개 통과·10개 선택 스모크 건너뜀, analyze와 Windows debug 빌드·시작 통과.
  변경 파일 포맷은 통과하며 전체 포맷은 기존 테스트 4개의 차이로 실패한다.
  검증 SDK는 Flutter 3.47.2/Dart 3.13.2다.
  기준 Flutter 3.44.8/Dart 3.12와 실제 SC:R 적용 검증은 남는다.
- 기존 사용자 변경은 별도로 보존한다. 다음 개발 계획은 자체 제작 게임 시나리오
  준비와 실제 SC:R 설정 적용 검증을 유지한다.

### 첫 게임 검증 맵 준비 (2026-09-13)

- [공중 유닛 설정 비교](SETTINGS_GAME_SMOKE.md)의 기준/변경 맵 생성기를 추가했다.
  레이스·스카웃의 체력·실드·방어력과 사이언스 베슬/오버로드 대조군을 배치했다.
  첫 단계는 선택 화면의 수치 관찰이며 생산·연구·전투 동작 검증용은 아니다.
- 표준 VCOD를 대조하고 두 맵의 예상 UNIx 변경 및 나머지 섹션 동일성, 시작 위치,
  대조군 기본값·실제 MPQ 생성/재열기를 확인했다. 기존 EUD fixture도 그대로다.
- 집중 테스트 5개 통과. 전체 테스트 529개 통과·11개 선택 스모크 건너뜀.
  analyze 통과, 변경 파일 포맷 통과. 전체 포맷은 기존 테스트 4개의 차이가 남는다.
  Flutter 3.47.2/Dart 3.13.2 사용, 기준 3.44.8/3.12는 미검증이다.
- 파일은 build/manual-settings-air-stats에 생성했고 해시·실행 방법·예상값을 안내서에
  기록했다. 앱/네이티브 코드 변경이 없어 UI 빌드를 반복하지 않았다.
- 실제 SC:R 시작·수치 관찰은 사용자의 확인이 필요하다. 아직 게임 실행 성공이나
  M6.3 완료를 주장하지 않는다. 이후 생산·연구·전투 및 플레이어/세력 사례를 추가한다.

### 사용자 게임 확인과 전투·테크 비교 사례 (2026-09-14)

- 사용자가 첫 공중 유닛 기준/변경 맵의 테스트 완료를 보고했다. 해당 사례만 완료로
  기록하며 상세 수치·게임 build·스크린샷 미제공 사실과 전체 M6.3 검증 범위를 구분한다.
- [무기·업그레이드·클로킹 비교](SETTINGS_COMBAT_SMOKE.md)를 추가했다. --combat
  생성 경로에서 앱의 도메인 편집기로 무기 피해량·함선 무기 시작 레벨·클로킹 연구
  완료/에너지와 Player 1 맵 상속을 설정한다. Player 2의 개별 값은 보존한다.
- 집중 테스트 7개 통과: 두 종류 비교 맵의 golden bytes, 나머지 섹션·Player 2 보존,
  실제 MPQ 생성/재열기와 기존 EUD fixture 확인. 전체 테스트 531개 통과·11개 선택 스모크 건너뜀.
  analyze와 변경 파일 포맷은 통과. 전체 포맷은 기존 테스트 4개의 차이가 남는다.
  Flutter 3.47.2/Dart 3.13.2로 검증했고 기준 3.44.8/3.12는 미검증이다.
- build/manual-settings-combat에 두 맵과 안내서를 생성했고 해시·예상값을 기록했다.
  앱/네이티브 코드 변경은 없어 Windows 빌드를 반복하지 않았다.
- 실제 게임의 무기·업그레이드·클로킹 결과는 확인 대기이며 생산 허용·연구 비용/시간,
  플레이어·세력의 나머지 게임 사례도 후속 범위다. M6.3 전체는 진행 상태를 유지한다.

## EUD 도구 동봉·다중 언어 확장 계획 (2026-09-15)

사용자 요청에 따라 [상세 확장 계획](EUD_TOOLCHAIN_LANGUAGE_PLAN.md)을 추가했다.
계획 수립만 완료했으며 기능 구현 상태는 바꾸지 않는다. 다음 EUD 개발은 X1 도구
패키지·manifest·선택 정책부터 진행하고 X2에서 기존 M6.3.1 생성/게임 검증을 연결한다.

- [ ] X1 검증된 euddraft 동봉, 기본/외부 도구 선택, 고지·무결성·깨끗한 PC 검증
  - [x] 2026-09-25 관리형 0.10.2.5-editor.1 실제 동봉·앱 기본 선택·86개 파일 신뢰 목록 연결. updater 대체 소스와 고지 보완, 재현/변조 차단 및 PATH 없는 실제 빌드·취소·시간 제한·설치 불변성 검증. [동봉 기록](EUD_BUNDLED_TOOL.md).
  - [ ] 남은 배포 인수: 깨끗한 Windows·읽기 전용 ACL·네트워크 차단 환경 검증. 현재 개발 PC 결과로 이를 완료 처리하지 않는다.
  ...
  - 2026-09-16 빌드 실행 전/승격 전 도구 버전·경로 재검사 및 설치 불변성 스모크 추가. 이전 실행 복사본의 자동 업데이트 발견으로 고정 버전 검증 기록 정정; 업데이트 차단·읽기 전용 인수는 미완료 ([조사 기록](research/euddraft-0.10.2.5/AUTO_UPDATE_REVIEW.md)).
  - 2026-09-16 핵심 도구 파일 SHA-256을 준비/실행 전/승격 전에 비교해 동일 버전 교체를 차단. 634개 회귀 및 통제 조건의 실제 빌드 통과. 외부 설치 전체 인증·자동 업데이트 차단은 미완료.
  - 2026-09-16 EUD Tools 설치 폴더 Browse 추가: 선택 초안/저장 분리, 취소·실패·창 닫기 회귀. 전체 639개 통과, 정적 분석/C++ 컴파일 통과. Windows 전체 build/run은 SDK 업데이트 오류로 재검증 대기.
- [ ] X2 선언적 설정의 결정적 생성, 실드 초기화·게임 검증, 안전 빌드 연결 (M6.3.1 공통)
  - [x] 후보 5개 필드의 정렬된 비실행 JSON 생성 명세와 EUD Project 미리보기 (2026-09-25, [범위](EUD_GENERATION_PREVIEW.md)). 실제 소스 생성·hook 순서·컴파일·게임 검증은 아직 미완료.
- [ ] X3 epScript/Python entry·진단·프로젝트 마이그레이션 지원
- [ ] X4 Lua EUD 제한 문법·변환기·원본 위치 진단·비교 검증
- [ ] X5 동봉 도구 업데이트/복구·오프라인 배포·지원표 검증 (M8 공통)

### 2026-09-17 사용자 요청: Settings/EUD 시각 정보·연관 선택

- [x] 설정 탭 구분 아이콘, 로컬 유닛 이미지·이름/ID·무기/서브유닛 카드 추가.
- [x] 유닛 선택 시 직접 지상→공중→서브유닛 무기를 Settings/EUD 편집 대상으로 자동 선택. 데이터/초안 자동 변경 없음.
- [x] 빠른 선택·설치 변경의 늦은 결과 폐기, 무기 없음/조회 실패 안내, 실제 로컬 이미지·참조 읽기 검증.
- [x] Units/Availability 선택을 양방향 공유. 최초 탭 진입·서브유닛 선택 연동, 유닛별 초안/플레이어/맵 바이트 보존과 닫기 초기화 회귀 추가.
- [x] Settings/EUD에서 원래 유닛을 유지하며 모든 연결 서브유닛의 지상·공중 무기를 선택. 출처 이름/ID, 순환·중복 경로 회귀 검증 추가.
- 업그레이드·테크 자동 선택 및 게임 명령 버튼 아이콘 전체 지원은 별도 데이터 연결이 필요하다.
- 자세한 사용 범위와 검증은 [Map Settings](MAP_SETTINGS_UI.md) 및 [EUD 확장](EUD_EXTENSIONS.md)에 기록한다.
...
- [x] 실드 활성·최대량, 무기 최소·최대 사거리·피해 유형 후보 매니페스트와 ID/자료형/저장 범위/enum 구조 검증 구현 (소스 확인, 런타임 미검증)
- [ ] euddraft/eudplib·SC:R 실제 호환성, 사거리 단위 변환·게임 허용 범위·필드 조합 검증 확정
- [x] 선언적 v1 스키마·맵 경로/해시 참조·프로젝트 파일 Save As/다시 읽기·버전 거부·dirty/Undo 기반 구현 (ADR-0009)
- [x] EUD Project 탭·Windows 파일 선택·현재 맵 fingerprint 대조·명시적 재연결·미저장 종료 확인 구현 (필드 값은 읽기 전용)
- [x] 외부 바이트 revision 검사·복구 백업·기존 프로젝트 Save UI 연결 (568개 테스트 통과, 향후 스키마 변경 시 migration 구현)
- [x] EUD 프로젝트의 유닛/공유 무기 정적 영향 분석과 이름·ID 목록 — 직접/서브유닛 참조 구분, 미조회와 참조 없음 구분, 설치·맵 변경 시 출처 무효화 (2026-09-15)
- [x] 현재 후보 5개 필드의 CHK 기준값·EUD 적용 예정 값/출처 표시 및 프로젝트 충돌 시 미확정 처리 — 최대 실드 UNIS/UNIx 비교, 게임 기본값 미확인 구분, 맵 검증 연결 (2026-09-15; DAT 기본 수치 조회와 실제 게임 적용은 후속 범위)
- [x] 무기 ID·기본 이름 선택, 최소·최대 사거리(원시 정수)와 피해 유형 선언적 편집 UI — 일괄 검증·적용·제거·취소·Undo·프로젝트 저장 연결 (2026-09-15; 생성 빌드·게임 적용 미연결)
- [x] 유닛의 지상/공중 무기 참조에서 해당 EUD 무기 편집으로 이동 — 빈 프로젝트 조회, None 구분, 공유 ID 편집, 출처 변경 시 적용 차단 (2026-09-15)
- [x] 유닛별 실드 활성/비활성·최대량 선언적 편집 UI — CHK override 명시, 필드별 제거·검증·Undo·저장, 배치 UNIT 바이트 보존 (2026-09-15)
- [ ] 기존 배치/신규 생성 유닛의 실드 초기화 정책을 실제 euddraft/SC:R로 검증하고 생성 빌드에 연결 (현재 타입 설정만 저장, CUnit 초기화·보충·클램프·반복 쓰기 미구현)
  - 다음 작업 준비: [실드 초기화·생성 빌드 준비](EUD_SHIELD_BUILD_PREPARATION.md) — 도구 확인, 비교 맵 관찰표, 생성기/파이프라인 연결 순서 (2026-09-15)
...
- [x] 현재 맵/설정 스냅샷에서 결정적 테스트 소스·manifest 생성, 사용자 entry 보존과 hook 실행 순서 소스/컴파일 검증 (2026-09-26; 게임 관찰 별도)
- [x] 명시적 테스트 빌드를 SafeEudBuildPipeline에 연결, 별도 출력·원본 기준 반복 빌드·stale 프로젝트/맵 결과 거부 구현 (2026-09-26)
...
### 2026-09-26 설정별 EUD 후보 필드 확대

- [x] 8개 분류·61개 후보 필드 registry: Unit/Weapon/Flingy/Upgrade/Tech/Player/Sprite/Image, 참조 ID·부분 음성 테이블·자료형·enum·예약 비트 검증.
- [x] 공통 분류 편집·이름/ID 검색·다중 초안·일괄 적용/제거·취소·Undo/Redo·프로젝트 저장. 기존 실드/무기 편집과 동일한 선언 데이터 사용.
- [x] 사거리/스플래시/음성 범위 관계 및 명시적 CHK override 검증. 업그레이드/테크 비용 CHK 기준값·기본값/미확인 구분.
- [x] 새 테이블의 잘못된 무기 영향 조회 방지, 전역/플레이어 범위 및 공유 영향 미확인 표시.
- [x] 동봉 euddraft 0.10.2.5-editor.1에서 모든 필드·enum과 경계 대상의 실제 API 컴파일 검사 통과.
- [ ] 실행 소스 생성기·hook 순서·안전 빌드 연결, 공유 그래픽 영향 조회, 실제 SC:R/멀티플레이 검증. 위 결과만으로 M6.3.1/M6.3.2 전체를 완료 처리하지 않음.
- 세부 필드·사용법·근거·제약: [설정별 EUD 필드 확대](EUD_EXPANDED_FIELDS.md). 필드 확대 시점에는 Build 차단; 아래 후속 작업에서 명시적 테스트 빌드를 연결함.

### 2026-09-26 선언적 EUD 생성·테스트 빌드 연결

- [x] 61개 registry 필드의 결정적 Python 시작 플러그인과 프로젝트/맵/소스 해시 manifest 생성, UI 소스 미리보기 및 빌드 로그 기록.
- [x] 설정 전용(소스 파일 불필요) 및 사용자 epScript 결합 빌드. 생성 설정 → 사용자 시작 hook → 일반 트리거 순서의 공식 소스 확인·실제 컴파일 로그 검증.
- [x] 명시적 미검증 테스트 빌드 선택, 맵 binding 검사, 준비 후 프로젝트/맵 변경 거부, 기존 SafeEudBuildPipeline의 임시 출력·MPQ/CHK 검증·승격 연결.
- [x] 반복 빌드 원본 보존, 잘못된 맵 해시 거부, 설정 전용 실제 빌드 및 자체 제작 비교 맵 5종 생성.
- [ ] SC:R 실드/사거리/피해·이동·연구·그래픽 수치 관찰, 변신/생성 유닛과 멀티플레이 검증. 타입-only 실드 정책은 현재량 보충/클램프를 하지 않으며 검증 완료 정책이 아님.
- 사용법·비교 맵·관찰표: [EUD 생성 빌드 검증](EUD_GENERATED_BUILD_VALIDATION.md), 결정: [ADR-0012](decisions/0012-generated-eud-test-builds.md). X2/M6.3.1 전체 완료는 게임 검증 이후다.
...
2026-09-27 [일반 트리거 첫 구현](TRIGGER_EDITOR.md): TRIG 탭, 레코드 추가·복제·삭제·순서 변경, P1~P8 실행 소유자, 조건 8종·액션 12종, 초안·Undo/Redo와 native MPQ Save As 재열기를 구현했다. 게임 실행, 전체 조건·액션, 브리핑은 남아 있다.

- [x] 단일 TRIG 레코드 모델·8개 조건/12개 액션 UI·편집 슬롯 참조 검증·Undo/Redo·실제 MPQ 왕복

- [x] 일반 `TRIG`와 문자열/로케이션/속성 슬롯 인자 참조 모델 구현 (전체 리소스 사용처는 M6.5 연계 항목)
- [x] 플레이어/조건 22종/액션 57종 구조 편집 UI
- [x] 레코드·슬롯 복사/이동, 활성/비활성, 일괄 소유자 변경
- [x] 원시 트리거 바이트 검사 뷰 (읽기 전용 16진수)
- [x] 지원하지 않는 조건/액션의 무손실 보존 (opcode/EUDX 읽기 전용, 레코드 전체 삭제는 명시적 확인)
- [x] 알려진 일반 슬롯의 수치·enum·문자열/로케이션/속성 참조 검증 및 편집 TRIG 저장 전 차단 (MPQ 사운드 존재 검사는 M6.5)
- [x] 일반 트리거와 EUD 확장 경계: EUDX·범위 밖 Deaths/Set Deaths·미지원 opcode 읽기 전용, 원시 레코드 보존
- [x] 일반 조건 22종·액션 57종 지원 표, 플레이어/유닛/로케이션/자원/점수/AI 4문자 ID 인자 편집 및 독립 바이트 기준 비교
- [x] SWNM 이름의 문자열 분리 추가, Create Unit with Properties의 UPRP/UPUS 64개 슬롯·공유 참조 수·예약 비트 보존·Undo 관리
...
2026-09-27 일반 트리거 구조 편집 확장 완료: [지원표·사용법·검증](TRIGGER_EDITOR.md). 735개 테스트 통과·23개 환경 조건부 skip, native MPQ 속성/스위치/원시 레코드 왕복, Windows 빌드·시작 확인. 실제 게임 실행은 미확인이며 M7 전체 완료로 처리하지 않는다.
