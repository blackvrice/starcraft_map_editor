...
위 해시는 공개 메타데이터이며 로컬 다운로드 바이트와 대조한 결과가 아니다.
전체 파일별 해시, 네이티브 구성요소 및 포함된 플러그인/라이선스는 실제 ZIP을 검토하여
확정해야 한다. 확인 전 production manifest나 재배포 가능 판정을 만들지 않는다.
...
1. ZIP 다운로드 바이트와 공개 해시를 대조하고 전체 파일·의존성·고지 목록 작성.
