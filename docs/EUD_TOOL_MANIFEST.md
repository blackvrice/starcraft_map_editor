...
2026-09-16 후속 검사 보완: `lib/freezeMpq.pyd`는 보호 옵션이 꺼져도 필수다.
프로젝트/사용자/동봉 후보에서 누락·빈 파일·디렉터리를 먼저 차단한다. 조사 manifest에
누락이 반영되어 있더라도 필수 파일 검사가 생략되지 않는다. 공식 ZIP을 로컬에 추출해
외부 설치 검사와 전체 manifest 대조를 검증했으며 실행 파일은 실행하지 않았다.
...
기본 테스트는 직접 만든 실행 불가능한 파일들로 진행하며 환경 변수로 공식 설치의
읽기 전용 검사도 실행할 수 있다. 파일 검사 통과를 실제 컴파일이나 게임 적용 성공으로 표시하지 않는다.
