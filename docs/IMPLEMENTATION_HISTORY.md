...
M0~M6.1의 완료 당시 체크리스트, 검증 결과와 인수 조건을 보존한다.
현재 상태와 다음 작업은 [개발 계획](DEVELOPMENT_PLAN.md)을 따른다.
여기의 테스트 수와 도구 버전은 각 기록 당시의 값이며 현재 검증 결과가 아니다.
완료 이후의 보완 작업은 개발 계획에서 추적한다.
