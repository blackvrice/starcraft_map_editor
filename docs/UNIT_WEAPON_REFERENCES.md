...
서브유닛 무기 선택 목록은 원래 유닛의 지상→공중 슬롯 다음에 subunit1과 subunit2를
깊이 우선으로 탐색한다. 각 유닛은 한 번만 방문하므로 순환·여러 경로의 중복 연결도
종료한다. 동일 무기의 지상/공중 슬롯은 출처를 구분해 유지한다. 자동 선택 우선순위는
기존과 같고, 모든 연결을 담은 목록은 변경 불가능하다. 카드의 서브유닛 무기 버튼은
원래 유닛을 유지한다. 도메인 테스트에서 탐색 순서·순환·중복 경로·동일 무기 슬롯을,
위젯 테스트에서 무기 전환과 유닛 선택 보존을 검증한다.
