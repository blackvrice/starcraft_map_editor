...
- 사용자 확인: 2026-09-23 실제 성능 직접 확인 완료 보고를 받았다. 자동 계측 수치나
  GPU 메모리 측정치가 추가로 제공된 것은 아니므로 아래 원시 기록과 구분한다.
...
### 보유 썸네일 상한과 재요청 (2026-09-25)

Tile의 현재 controller 상태가 가진 RGBA 합계는 기본 16MiB 이하다. 초과하면 최근
사용 순서가 오래된 픽셀만 해제한다. metadata·검색·배치 선택은 남고, 보이는 셀과
상세 미리보기가 필요한 타일을 요청하면 최대 128개씩 한 프로세스 요청 경로로 다시
읽는다. 설치/맵/종류 변경이나 종료 뒤 응답은 버리며, storage/helper identity와
ID/tileset 불일치·실패는 같은 종류를 재로딩하기 전까지 자동 반복하지 않는다.

한 종류의 목록만 보유한다. Unit은 228×16KiB=3.5625MiB, Sprite는
517×16KiB=8.078125MiB의 기존 항목별 상한과 고정 ID 개수로 16MiB 아래에 머문다.
Doodad 목록에는 RGBA가 없다. `retainedThumbnailBytes`는 현재 상태의 payload만
합산하며 metadata, 이전 상태를 외부에서 보유한 참조, atlas의 임시 버퍼와 GPU는
이 예산에 포함하지 않는다. 새 페이지/재요청 처리 중 임시 할당은 남는다.

회귀는 작은 예산에서 픽셀만 해제, 검색/배치 가능한 목록 보존, 중복 요청 병합,
필요 타일 재로딩, 실패 반복 차단과 설치 변경 뒤 늦은 응답 차단을 확인한다.
