...
빌드 실행 전과 출력 승격 전에는 준비 계획의 도구 버전·실행 파일·경로 출처를
재검사한다. 변경 또는 검사 실패는 `EUD_BUILD_TOOL_CHANGED`로 차단한다.
준비 시 기록한 실행 파일·VERSION·필수 동반 파일·버전별 Python DLL의 SHA-256도
비교해 동일 버전 파일 교체를 감지하고, 진단에 변경된 상대 파일명을 기록한다.
동봉 도구는 두 시점 모두 manifest 무결성 검사를 유지한다. 외부 도구의 자동
업데이트 자체를 차단하는 기능은 아직 없다. 종료 후 비동기 교체 가능성과 실제
스모크의 통제 조건은 [자동 업데이트 조사](research/euddraft-0.10.2.5/AUTO_UPDATE_REVIEW.md)를 따른다.
...
미구현 설계를 [EUD 확장 설정 설계](EUD_EXTENSIONS.md#1-계획된-유닛무기-eud-확장-설정)로 분리했다.
M6.3.1에서 일반 CHK 설정과 구분하여 구현하며 이 문서의 안전한 빌드 경계를 재사용한다.

## 15. 계획된 설정별 EUD 확장 탭

설정별 범위, 공통 탭과 실행 규칙은 [EUD 확장 설정 설계](EUD_EXTENSIONS.md#2-계획된-설정별-eud-확장-탭)를 따른다.
초기 설정은 M6.3.2, 조건부·런타임 실행은 M7.1의 미구현 범위다.
